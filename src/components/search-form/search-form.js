...
const SearchForm = () => {
    ...
    let specialties = []
     allPractitioners.forEach(e => specialties.includes(e.specialty) ? null : specialties.push(e.specialty))   
    const handleChange = (e) =>{
        if(e.target.value.length >0){
            setshowSpecialties(true)
            let val = e.target.value.toLowerCase()
            let filteredSpecialties = specialties.filter(e => e.toLowerCase().includes(val))           
            let filteredPractitioners = allPractitioners.filter(e => e.specialty.toLowerCase().includes(val))           
            // console.log(filteredPractitioners)
            setpractitioners(filteredPractitioners)
            setAllSpecialty(filteredSpecialties)
        ...
        <div className="container ">
            ...
            <div class="form-row">
              ...
              <svg width="20" height="20" viewBox="0 0 20 20" class="searchbar-input-icon pt-1"><path d="M19.6 16.92c.48.6.53 1.12.14 1.58l-1.2 1.2c-.58.48-1.16.48-1.72 0l-4.8-4.86a7.6 7.6 0 01-3.96 1.1 7.65 7.65 0 01-5.64-2.44A7.8 7.8 0 017.82 0C10 0 11.9.8 13.5 2.42a7.74 7.74 0 011.23 9.74l4.86 4.76zM2.36 7.86c0 1.5.58 2.8 1.74 3.96 1.16 1.16 2.48 1.73 3.96 1.73s2.76-.54 3.8-1.63a5.35 5.35 0 00-.09-7.76A5.44 5.44 0 007.82 2.4c-1.52 0-2.8.55-3.8 1.64a5 5 0 00-1.65 3.8z"></path></svg>
                <input ref={specialtyRef}  name="specialty" autoComplete="off" onChange={handleChange}class="form-control searchbar  form-control-lg" placeholder="Doctor, establishment, specialty…" required/>
                {showSpecialties && <SearchDropdown practitioners={practitioners} specialties={allSpecialty} handleSelect={handleSelect} />}
              </div>
              ...
              <svg width="16" height="20" viewBox="0 0 16 20" class="searchbar-input-icon">
                ...
                <input  class="form-control searchbar no-radius form-control-lg" autoComplete="off" placeholder="location" name="location" onChange={(e) => setlocation(e.target.value)} required/>
              </div>
